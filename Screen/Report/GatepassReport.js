...
const GatepassReport = ({ route }) => {
  ...
  const fetchRfidData = async () => {
    try {
      const response = await axios.get(`https://macts-backend-mobile-app-production.up.railway.app/gatepass_tapHistory/${user_id}`);
    ...
    return (
      <View style={styles.data}>
        ...
        <View style={[styles.cell, { width: responsiveSize(150) }]}>
          <Text style={styles.cellText}>{item.gatepass_tupID}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(200) }]}>
          <Text style={styles.cellText}>{`${item.gatepass_firstname} ${item.gatepass_middlename} ${item.gatepass_lastname} `}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(150) }]}>
          <Text style={styles.cellText}>{item.gatepass_course}</Text>
        </View>
