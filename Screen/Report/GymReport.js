...
const GymReport = ({ route }) => {
  ...
  const fetchRfidData = async () => {
    try {
      const response = await axios.get(`https://macts-backend-mobile-app-production.up.railway.app/gym_tapHistory/${user_id}`);
    ...
    return (
      <View style={styles.data}>
        ...
        <View style={[styles.cell, { width: responsiveSize(150) }]}>
          <Text style={styles.cellText}>{item.gym_tupID}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(200) }]}>
          <Text style={styles.cellText}>{`${item.gym_firstname} ${item.gym_middlename} ${item.gym_lastname} `}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(150) }]}>
          <Text style={styles.cellText}>{item.gym_course}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(100), paddingLeft: responsiveSize(16) }]}>
          <Text style={styles.cellText}>{item.gym_section}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(200) }]}>
          <Text style={styles.cellText}>{item.gym_InHistoryDate}</Text>
        </View>

        <View style={[styles.cell, { width: responsiveSize(200) }]}>
          <Text style={styles.cellText}>{item.gym_OutHistoryDate}</Text>
        </View>
