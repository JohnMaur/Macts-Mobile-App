...
//     RegistrarSocket.on('tagData', data => handleTagData(data, 'Registrar'));
...
//   useEffect(() => {
//     if (currentTap) {
//       if (setting === 'Registrar') {
//         RegistrarTapHistory(currentTap);
//       }
//     }
//   }, [currentTap, setting]);
  ...
  useEffect(() => {
    ...
    const handleTagData = (data) => {
      ...
      if (data.excessiveTap && data.tagData === studentInfo?.tagValue) {
        ...
        setCurrentTap({ ...studentInfo, taggedAt: new Date().toLocaleString('en-US') });
        setSetting('Registrar');
      } else {
        console.log("Tag value doesn't match the student information.");
      }
    };

    RegistrarSocket.on('tagData', handleTagData);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Text style={styles.modalText}>
            You've already tapped your RFID card. Please wait for 30 seconds before tapping again.
            </Text>
            <TouchableOpacity
              style={{ ...styles.openButton, backgroundColor: "#2196F3" }}
              onPress={() => setShowExcessiveTappingModal(false)}
