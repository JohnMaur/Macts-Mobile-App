...
const Gym = ({ route }) => {
  ...
  useEffect(() => {
    // const gymSocket = io('http://192.168.92.90:3030');
    const gymSocket = io('wss://macts-backend-gym.onrender.com');
  ...
  const gymTapHistory = async (data) => {
    try {
      const response = await axios.post('http://192.168.92.90:2525/gym_history', {
